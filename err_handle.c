...
void outputErr(int32_t flags, int32_t errnum, const char* fstring,/*#{{{*/
               va_list argList)
{
#define _BUFF_SIZE_ 500 /* i need to learn specifically the difference when putting
                           the #define in a function. I know it works, just not
                           the preprocessor details. Should i undefine this at
                           the end of the function? does that make it local to
                           this function? (i think it does, not confirmed) */
